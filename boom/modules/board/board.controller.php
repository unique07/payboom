...
class boardController extends board
{
	...
	/**
	 * @brief blind the document
	 **/
	function procBoardBlindDocument()
	{
		...
		$oDocument = $oDocumentModel->getDocument($document_srl);
		// check protect content
		if($this->module_info->protect_content=="Y" && $this->grant->manager==false)
		{
			return new Object(-1, 'msg_protect_content');
		}
		...
		$output = $oDocumentController->blindDocument($document_srl, $this->grant->manager);
		if(!$output->toBool())
		{
			return $output;
		}
