...
class memberController extends member
{
		...
		switch($name)
		{
			...
			case 'bizno' : //사업자번호
				...
				if($member_srl && $logged_info->member_srl != $member_srl ) {
					if($value) {
						$is_bizno = $this->RegNoCheck($value);
						if($is_bizno != true) {
							return new Object(0,'msg_chk_bizno');
						} else {
							$info = $oMemberModel->getMemberExtend($value);
							//echo $info->member_srl . " = " . $logged_info->member_srl;
							if($member_srl && $logged_info->member_srl != $member_srl ) return new Object(0,'msg_exists_bizno');
						}
					}
				}
				break;
	...
	function procMemberInsert()
	{
		...
		$args->biznm = Context::get('biznm');
		$args->bizno = Context::get('bizno');
		$args->masternm = Context::get('masternm');
		$args->phone = Context::get('phone');
		$args->cellphone = Context::get('cellphone');
		$args->post = Context::get('post');
		$args->address = Context::get('address');
		$args->address_sub = Context::get('address_sub');
		$args->bizproduct = Context::get('bizproduct');
		$args->biz_status = Context::get('biz_status');
		$args->biz_item = Context::get('biz_item');
		$args->grp = Context::get('grp');
	...
	function procMemberResendAuthMail()
	{
		...
		// Send a mail
		$oMail = new Mail();
		$oMail->useNaverAccount("unique@payboom.co.kr", "Sorimoa@9645");
	...
	function insertMember(&$args, $password_is_hashed = false)
	{
		...
		/*
		if(ereg("(^0[1-9]{1,2})-([1-9][0-9]{1,3})-([0-9]{4})", $args->phone)) {
		} else {
		   return new Object(-1,'msg_chk_phone');
		}
		
		if(ereg("(^0[1-9]{1,2})-([1-9][0-9]{1,3})-([0-9]{4})", $args->cellphone)) {
		} else {
		   return new Object(-1,'msg_chk_mobile');
		}
		*/
		...
		if($args->grp == "02") {
			...
			$is_bizno = $this->RegNoCheck($args->bizno);
			if($is_bizno != true) {
				return new Object(-1,'msg_chk_bizno');
			} else {
				$bizno = $oMemberModel->getMemberExtend($args->bizno);
				if($bizno) return new Object(-1,'msg_exists_bizno');
			}
			
			$args->group_srl_list = "412";
		} else {
			$args->group_srl_list = "3";
		}
		...
		$extend_args = new stdClass();
		
		$extend_args->member_srl = $args->member_srl;
		$extend_args->user_id = $args->user_id;
		$extend_args->authno = $args->authno;
		$extend_args->biznm = $args->biznm;
		$extend_args->bizno = $args->bizno;
		$extend_args->masternm = $args->masternm;
		$extend_args->phone = $args->phone;
		$extend_args->cellphone = $args->cellphone;
		$extend_args->post = $args->post;
		$extend_args->address = $args->address;
		$extend_args->address_sub = $args->address_sub;
		$extend_args->bizproduct = $args->bizproduct;
		$extend_args->biz_status = $args->biz_status;
		$extend_args->biz_item = $args->biz_item;
		
		$output = executeQuery('member.insertMemberExtend', $extend_args);
		if(!$output->toBool())
		{
			$oDB->rollback();
			return $output;
	...
	function updateMember($args, $is_admin = FALSE)
	{
		...
		foreach ($extend_args as $k => $v) {
			if($v == "") {
				$extend_args->$k = "";
			}
		}
		$info = $oMemberModel->getMemberExtend($args->user_id, "user_id");
		if($info->member_srl != "") {
			$output = executeQuery('member.updateMemberExtend', $extend_args);
		} else {
			$output = executeQuery('member.insertMemberExtend', $extend_args);
		}
	...
	function deleteMember($member_srl)
	{
		...
		$output = executeQuery('member.deleteMemberExtend', $args);
		if(!$output->toBool())
		{
			$oDB->rollback();
			return $output;
